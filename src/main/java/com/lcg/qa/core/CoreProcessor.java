...
public class CoreProcessor {
    ...
    public List<String> analysis(String querySentence) throws Exception {
        ...
        resultList.addAll(Arrays.asList(finalPatternArr));
        return resultList;
    ...
    public  String queryAbstract(String querySentence) {
        ...
        for (Term term : terms) {
            ...
            if (termStr.contains("na")) {
            ...
            } else if (termStr.contains("nc")) {
                abstractQuery.append("nc ");
                ...
                abstractQuery.append("nj ");
                abstractMap.put("nj", word);
            }else if(termStr.contains("nt")){
                abstractQuery.append("nt ");
                abstractMap.put("nt", word);
            }
            else {
    ...
    public NaiveBayesModel loadClassifierModel() throws Exception {
        ...
        for (Map.Entry<Double, String> entry : seqWithSamples.entrySet()) {
            ...
            //遍历每一个具体的问句,将问句分词与词汇表对应的位置生成向量，每一种问题有好几种问法，所以这里用train_list
            //汇总所有问法产生的向量
            for (String sentence : sentences) {
                //将训练的句子转换为向量数组
                double[] array = sentenceToArrays(sentence);
                //具体问句对应的向量 key:问题类型序号; value: 具体某一问句向量
    ...
    public Map<Double,String> loadQuestionSamples(String path) throws IOException {
        ...
        Map<Double,String> seqWithSamples = new HashMap<>(25);
